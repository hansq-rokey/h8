...
$(document).ready(function(){
    $(document).on('click','.confirm',function(){
    	if($('#categoryModelSel').text()!='-选择-'&&$('#categoryModelSel').text()!=''){
    	   $('.pop').show();
    	}
    });
	$('.determine').on('click',function(){
		$('#modelIdHidden').val($("#modelId").val());
		$('#formatIdHidden').val($("#formatId").val());
		$('#modelNameHidden').val($(".CategoryModel").text());
		$('#formatNameHidden').val($(".CategoryFormat").text());
		$('#batchHidden').val($(".CategoryBatch").text());
		$('#numHidden').val($("#num").val());
    	$('.pop').hide();
    	$('#fwmForm').submit();
    }) 
})
  ...
  //二级数据
	...
	$.ajax({
 		   ...
 		   success: function(obj){
 			  ...
 			  var data = obj.data;
 			  for(var o in data){
 				 html = html+ "<li onclick='setcategoryModelFormatVal("+data[o].id+")' id='liumf"+data[o].id+"' _name='"+data[o].batchNumber+"'>"+data[o].batchNumber+"</li>";
 			  }
...
<body>
    <section>
        ...
        <span class="select-specifica">生产数量：</span>
...
<form action="/fwm/create.html" method="post" id="fwmForm">
	<div class="popbg"></div>
    <div class="layel">
    ...
    <input type="hidden" name="modelId" id="modelIdHidden" />
    <input type="hidden" name="formatId" id="formatIdHidden" />
    <input type="hidden" name="modelName" id="modelNameHidden" />
    <input type="hidden" name="formatName" id="formatNameHidden" />
    <input type="hidden" name="batch" id="batchHidden" />
    <input type="hidden" name="num" id="numHidden" />
</form>
</div>
</body>
</html>